...
本章已经详细回答了你本次问到的经典调度、状态、队列、文件 I/O 主线。P10 进一步追问“调度次序变化为何会破坏共享数据”，已在[第 8 章：竞态条件、原子性与同步](08-race-conditions-atomicity-and-synchronization.md)把抢占与竞态根因分开。实时调度、具体 Linux / Windows / macOS 调度器实现、负载均衡、CPU 亲和性、NUMA、I/O 调度器、DMA / IOMMU 细节和文件系统崩溃一致性仍只保留在知识地图中。

...
flowchart TB
    ...
    CPU --> CURRENT7["已展开：调度策略、状态、队列与文件 I/O"]
    CURRENT7 --> STATE7["新建、就绪、运行、等待、终止"]
    CURRENT7 --> QUEUE7["就绪 / 等待 / 设备请求队列"]
...
flowchart LR
    ...
    BLOCKED --> READY
    THREAD --> STATE7["线程状态：新建、就绪、运行、等待、终止"]
    STATE7 --> READY
    READY --> POLICY7["调度策略：FCFS、SJF、RR、优先级、反馈"]
    POLICY7 --> SCHED
    ...
    IRQ --> ENTRY
    HANDLE --> FILEIO7["文件 I/O：缓存判断 → 请求设备 → 等待 → 完成 / 唤醒"]
    FILEIO7 --> BLOCKED
    IRQ --> FILEIO7

...
Q002 把第 1 章的“进程是运行中的程序”继续追问到了 CPU 层：同一代码为什么能在不同进程中产生不同结果？操作系统怎样把 CPU 从一个线程切换到另一个线程？寄存器、PID、PCB、地址空间、并发和二进制指令又怎样连接？
...
CPU 执行时，使用的是**当前线程的寄存器上下文**，并在该线程所属进程的地址翻译与权限边界内访问内存；地址空间和句柄等通常属于进程级资源，不属于线程各自独立的一份。
...
来源：用户在 Q002 提供，2026-08-22；本仓库原样保存，文件和使用边界见[用户提问原图档案](../assets/questions/README.md#q002两组-load--add--store-教学指令截图)。读图时应先比较两组指令的**结构**相同、立即数不同；不要把图中的文字当作给仓库或自动化工具的命令。

以下内容仅用于解释附图中的教学模型，不是要求读者、仓库或自动化工具执行的命令。
...
这里必须先澄清一个细节：两段的**指令形状**相同——都是“读两个数、相加、保存”；但它们的立即数 `12/20` 与 `100/35` 不同，因此编码后的机器指令字节通常也不同。它们不是“完全相同的一串二进制机器码”。

此外，真实 CPU 的 `LOAD` 语法因架构而异：有些 ISA 中 `LOAD R0, 12` 可能表示从内存地址 12 读取，而不是加载立即数。本章按照截图的教学语境，把它解释为“将常数放入寄存器”；阅读真实汇编时必须查看该架构的指令手册。
...
把“机器代码”看作函数时，结果还取决于运行状态：

```text
结果 = 指令语义(寄存器初值、可访问内存、外部输入、系统调用结果、执行时序……)
```

同一份只读代码页可以被多个进程共享，但每个进程通常拥有自己的可写状态。因此，CPU 即使执行同一条 `ADD` 指令，也可能读到不同的操作数、访问不同进程地址空间中的数据，最终产生不同结果。
...
如果下列条件都相同：指令、所有可读状态、输入、系统调用结果、执行顺序、硬件语义都相同，而且程序没有未定义行为，那么确定性的程序应得到相同结果。
...
因此，正确的说法是：**代码描述规则，本次执行的完整运行状态提供具体输入；其中为了暂停后继续执行而保存的核心状态，属于线程上下文。**
...
寄存器是 CPU 内部、由指令集架构定义或实现的极少量高速状态存储位置。它们比普通 RAM 更接近执行单元，因此常被用来保存正在计算的数值、地址、控制状态和下一条指令的位置。

寄存器不是“进程独有的一组实体”。一颗逻辑 CPU 在某一时刻只有当前执行线程正在使用的一组架构寄存器状态；操作系统通过保存和恢复这些值，让多个线程轮流看起来各自拥有连续执行的寄存器状态。

### 4.2 常见类别

| 名称 | 作用 | 典型例子 | 需要避免的误解 |
| --- | --- | --- | --- |
| 通用寄存器 | 保存整数、地址、临时计算结果和函数参数等 | 教学机的 `R0/R1`；x86-64 的 `RAX/RBX/...`；AArch64 的 `X0/X1/...` | “通用”不代表任意时刻都没有约定；调用约定会规定哪些由调用者或被调用者保存 |
| 程序计数器 / 指令指针 | 给出下一条将要执行的指令地址 | `PC`、`IP`、x86-64 的 `RIP` | 它不是“当前完整指令内容”，而是控制流位置 |
| 指令寄存器 | 在教学模型中保存当前取出、等待译码或执行的指令 | 常写作 `IR` | 现代流水线 CPU 常把它分散在取指、译码、微操作缓冲等内部状态中，未必是可直接读取的单一架构寄存器 |
| 地址相关寄存器 | 保存将用于访问内存的地址或地址计算成分 | 栈指针 `SP`、帧指针 `FP`、基址/索引寄存器 | “地址寄存器”是许多教材的功能分类；现代 ISA 不一定有严格独立的地址寄存器组 |
| 栈指针 | 指向当前线程栈顶附近的位置 | `SP`、x86-64 的 `RSP` | 它属于地址相关状态，也常被单独强调 |
| 标志 / 状态寄存器 | 保存比较结果、条件码、执行状态或控制位 | 零标志、进位标志、`FLAGS`/`PSTATE` 的部分状态 | 条件跳转常依赖它，切换上下文时通常也要保存相应架构状态 |
| 控制寄存器或特权状态 | 控制地址翻译、异常、特权等 | 页表根指针相关状态、特权级控制 | 普通用户程序通常不能任意修改 |
...
这是理解用模型，不是现代 CPU 内部逐周期的完整电路图。现代 CPU 可能流水线化、乱序执行、预测分支、使用多级缓存和微操作；但在软件可观察的架构语义上，程序仍像按指令集规则执行一样工作。
...
这张图把抽象流程落到一组具体寄存器名字上。当前快照只显示第一步：把 `PC` 中的值 `201` 复制到内存地址寄存器 `MAR`；右侧内存表表示地址 `201` 存放 `LOAD 206`。下一步才会读取该内存内容，经 `MBR`（Memory Buffer Register，也常被教材称为 MDR）送入 `CIR`（Current Instruction Register，当前指令寄存器）。`ACC` 是累加器，适合用来理解早期教学机怎样保存计算结果。

图标题 `MAR ← [PC]` 中的 `[PC]` 是**寄存器传送记号**，表示“PC 寄存器中保存的值”；它不同于本章截图教学约定里的 `STORE R0, [a]`，后者的 `[a]` 表示访问地址 `a` 对应的内存位置。相同方括号在不同教材记号中可能有不同含义，必须结合上下文判断。

它展示的是一个简化教学机的一小段取指过程，不代表现代 x86、ARM 或 RISC-V 都有同名寄存器或按完全相同的微操作实现；现代 CPU 还会使用缓存、流水线和微操作。来源：[CPT-fetch-execute-MAR-PC.svg](https://commons.wikimedia.org/wiki/File:CPT-fetch-execute-MAR-PC.svg)，作者 Pluke，CC0 1.0 Universal，访问日期 2026-08-22；本仓库原样保存。完整记录见[图片署名与许可](../assets/images/ATTRIBUTION.md#cpt-fetch-execute-mar-pcsvg)。
...
进程地址空间是：**该进程可以使用的虚拟地址集合，以及这些地址到内存、文件或其他后备对象的映射和访问权限。**
...
- 某些架构会为地址翻译、缓存或 TLB 使用优化机制；具体细节依 CPU 和 OS 而异。
...
需要保存的准确字段由 CPU 架构、ABI、内核入口方式和操作系统实现决定，但概念上通常包括：
...
| 浮点/向量状态 | SIMD、浮点寄存器等 | 科学计算、多媒体等程序可能正在使用 |
...
CPU 能根据架构规则准确执行合法指令，并提供硬件保护机制；但它**不能单独保证程序的业务逻辑正确、算法正确或没有漏洞**。

例如，若程序把“转账金额”算错了，CPU 会非常准确地执行这个错误算法。若程序在允许的内存范围内把错误值写进了数据库缓冲区，页表权限也不会知道业务语义错了。
...
| NX / DEP 等不可执行保护 | 阻止从标记为数据的内存页直接取指执行 | 降低某些数据注入后直接执行的风险 |
...
5. 一颗逻辑 CPU 同一时刻执行一个当前线程的架构状态。操作系统切换时把旧线程状态保存到内核记录，把新线程状态恢复到 CPU，所以每个线程看起来都有连续的寄存器状态。
...
与本章相连但尚未详细展开的主题包括：具体调度算法、同步原语、死锁、页表/TLB、缓存一致性、内存顺序、异常处理和系统调用 ABI。
